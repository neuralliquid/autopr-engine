# Enhanced Whitespace Fixer Pre-commit Hook (PowerShell)
# Automatically fixes whitespace and line ending issues and re-stages files
[CmdletBinding()]
param(
...
Write-Host "Enhanced Whitespace Fixer - Pre-commit Hook" -ForegroundColor Green
...
foreach ($File in $Files) {
    ...
    # Run whitespace fixer with default settings (LF line endings, no tab conversion)
    try {
        $Result = & python __main__.py $FullPath 2>&1
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Warning "Whitespace fixer failed for $File`: $Result"
        }
    }
    catch {
