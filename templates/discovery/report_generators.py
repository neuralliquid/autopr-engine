...
class ReportGenerator(ABC):
    ...
    @abstractmethod
    def generate_report(self, metrics: QualityMetrics, analysis: Optional[Dict[str, Any]] = None) -> str:
        """Generate a quality assurance report."""
        pass
    
    @abstractmethod
    def generate_batch_report(self, template_metrics: List[QualityMetrics], batch_analysis: Optional[Dict[str, Any]] = None) -> str:
...
class JSONReportGenerator(ReportGenerator):
    ...
    def generate_report(self, metrics: QualityMetrics, analysis: Optional[Dict[str, Any]] = None) -> str:
        ...
        report_data = {
            ...
            'analysis_timestamp': metrics.analysis_timestamp.isoformat() if metrics.analysis_timestamp else None,
            'quality_metrics': {
    ...
    def generate_batch_report(self, template_metrics: List[QualityMetrics], batch_analysis: Optional[Dict[str, Any]] = None) -> str:
        ...
        report_data = {
            ...
            'summary': {
                ...
                'average_score': sum(m.overall_score for m in template_metrics) / len(template_metrics) if template_metrics else 0,
...
class MarkdownReportGenerator(ReportGenerator):
    ...
    def generate_report(self, metrics: QualityMetrics, analysis: Optional[Dict[str, Any]] = None) -> str:
        ...
        if metrics.analysis_timestamp:
            lines.append(f"**Analysis Date:** {metrics.analysis_timestamp.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        lines.append("")
        lines.append(f"- **Overall Score:** {metrics.overall_score:.1f}/100 (Grade: {metrics.quality_grade})")
        lines.append(f"- **Success Rate:** {metrics.success_rate:.1f}%")
        lines.append(f"- **Total Issues:** {len(metrics.issues)} ({metrics.errors_count} errors, {metrics.warnings_count} warnings)")
        lines.append("")
    ...
    def generate_batch_report(self, template_metrics: List[QualityMetrics], batch_analysis: Optional[Dict[str, Any]] = None) -> str:
        ...
        if template_metrics:
            ...
            lines.append(f"- **Templates with Errors:** {sum(1 for m in template_metrics if m.has_critical_issues)}")
            lines.append("")
        ...
        for metrics in sorted(template_metrics, key=lambda m: m.overall_score, reverse=True):
            ...
            status = "🔴 Critical" if metrics.has_critical_issues else "✅ Good" if metrics.overall_score >= 80 else "🟡 Needs Work"
            lines.append(f"| {template_name} | {metrics.overall_score:.1f} | {metrics.quality_grade} | {len(metrics.issues)} | {status} |")
...
<body>
    ...
    <h2>Template Overview</h2>"""
        ...
        for metrics in sorted(template_metrics, key=lambda m: m.overall_score, reverse=True):
            template_name = Path(metrics.template_path).name if metrics.template_path else "Unknown"
            status_class = "critical" if metrics.has_critical_issues else "good" if metrics.overall_score >= 80 else "warning"
...
def generate_report(metrics: QualityMetrics, format_type: str = "markdown", analysis: Optional[Dict[str, Any]] = None) -> str:
...
def generate_batch_report(template_metrics: List[QualityMetrics], format_type: str = "markdown", batch_analysis: Optional[Dict[str, Any]] = None) -> str:
