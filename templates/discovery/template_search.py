...
class TemplateSearchEngine:
    ...
    def search_templates(self, 
                        ...
                        use_case: Optional[str] = None) -> List[TemplateInfo]:
        ...
        if query:
            ...
            results = [
                ...
                if (query_lower in t.name.lower() or
                    ...
                    any(query_lower in feature.lower() for feature in t.key_features) or
                    any(query_lower in uc.lower() for uc in t.use_cases))
            ]
    ...
    def _calculate_similarity(self, template1: TemplateInfo, template2: TemplateInfo) -> float:
        ...
        if platform_overlap > 0:
            score += 0.2 * (platform_overlap / max(len(template1.platforms), len(template2.platforms)))
        
        # Use case overlap
        use_case_overlap = len(set(template1.use_cases) & set(template2.use_cases))
        if use_case_overlap > 0:
            score += 0.2 * (use_case_overlap / max(len(template1.use_cases), len(template2.use_cases)))
        
        # Feature overlap
        feature_overlap = len(set(template1.key_features) & set(template2.key_features))
        if feature_overlap > 0:
            score += 0.2 * (feature_overlap / max(len(template1.key_features), len(template2.key_features)))
