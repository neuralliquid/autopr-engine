    ...
    def _parse_template(self, yaml_file: Path) -> Optional[TemplateInfo]:
        ...
        try:
            ...
            metadata = {k: v for k, v in data.items() 
                       if k not in ['name', 'description', 'category', 'platforms', 'variables', 'platform_info']}
    ...
    def search_templates(self, query: str) -> List[TemplateInfo]:
        ...
        for template in self.templates:
            if (query_lower in template.name.lower() or 
                ...
                any(query_lower in platform.lower() for platform in template.platforms)):
...
"""
    ...
    def get_template_statistics(self) -> Dict[str, Any]:
        ...
        # Platform statistics
        platform_templates = self.get_templates_by_platform()
        stats['platforms'] = {platform: len(templates) for platform, templates in platform_templates.items()}
        ...
        if self.templates:
            stats['variables_stats']['avg_variables_per_template'] = len(all_variables) / len(self.templates)
...
def main():
    ...
    print(f"Average variables per template: {stats['variables_stats']['avg_variables_per_template']:.1f}")
