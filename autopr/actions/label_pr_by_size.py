...
class LabelPRBySize(Action[Inputs, Outputs]):
    ...
    async def run(self, inputs: Inputs) -> Outputs:
        try:
            ...
            insertions = int(subprocess.run(['grep', '-o', '[0-9]* insertions'], input=stats, capture_output=True, text=True).stdout.split()[0] or 0)
            deletions = int(subprocess.run(['grep', '-o', '[0-9]* deletions'], input=stats, capture_output=True, text=True).stdout.split()[0] or 0)
