    ...
    async def run(self, inputs: Inputs) -> Outputs:
        ...
        try:
            ...
            for package, details in licenses.items():
                ...
                package_licenses = re.split(r' OR | AND ', license_str.replace('(', '').replace(')', ''))
            ...
            return Outputs(success=False, forbidden_packages=[], log=f"Failed to parse license-checker output: {e}")

if __name__ == "__main__":
    from autopr.tests.utils import run_action_manually
    import re # Added for the main block
    asyncio.run(
        run_action_manually(
