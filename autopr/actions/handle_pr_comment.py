    ...
    try:
        ...
        if len(filtered_lines) < len(lines):
            with open(file_path, 'w') as f:
                f.write('\n'.join(filtered_lines))
            return {"success": True, "description": f"Removed {len(lines) - len(filtered_lines)} console.log statements"}
...
"""
    ...
    cmd = [
        ...
        "--title", title,
        "--body", body,
        "--repo", f"{inputs.repo_owner}/{inputs.repo_name}",
        "--label", "autopr-generated,from-pr-comment"
    ]
