...
class IssueCreatorOutputs(BaseModel):
    ...
    ai_notifications_sent: List[Dict[str, Any]]
    errors: List[str]
    success_count: int
...
class IssueCreator:
    def __init__(self):
        ...
        self.linear_token = os.getenv('LINEAR_API_KEY')
    ...
    def create_issues_and_tickets(self, inputs: IssueCreatorInputs) -> IssueCreatorOutputs:
        ...
        if inputs.notify_ai_tools and inputs.ai_assignments:
            for assignment_key, ai_tool in inputs.ai_assignments.items():
                try:
                    notification = self._notify_ai_tool(assignment_key, ai_tool, github_created, linear_created)
...
**Estimated Time:** 15-60 minutes depending on complexity
    ...
    def _notify_ai_tool(self, assignment_key: str, ai_tool: str, github_issues: List, linear_tickets: List) -> Dict:
    ...
    def _notify_charlie(self, assignment_key: str, github_issues: List, linear_tickets: List) -> Dict:
        """Notify CharlieHelps about TypeScript assignments"""
        
        # Find relevant Linear tickets for Charlie
        charlie_tickets = [
            ticket for ticket in linear_tickets 
            ...
            "attachments": [
                {
                    ...
                    "fields": [
                        ...
                        {
                            "title": "Tickets",
                            "value": "\n".join([f"• <{ticket['url']}|{ticket['title']}>" for ticket in charlie_tickets]),
                            "short": False
                        }
                    ]
                }
    ...
    def _notify_azure_sre(self, assignment_key: str, github_issues: List, linear_tickets: List) -> Dict:
        """Notify Azure SRE about performance/infrastructure issues"""
        
        infra_issues = [
            issue for issue in github_issues
            if 'performance' in issue.get('labels', []) or 'infrastructure' in issue.get('labels', [])
        ]
    ...
    def _notify_promptless(self, assignment_key: str, github_issues: List, linear_tickets: List) -> Dict:
    ...
    def _notify_testim(self, assignment_key: str, github_issues: List, linear_tickets: List) -> Dict:
        """Notify Testim about testing issues"""
        
        test_issues = [
            issue for issue in github_issues
            if 'testing' in issue.get('labels', [])
        ]
...
if __name__ == "__main__":
    ...
    sample_inputs = {
        ...
        "ai_assignments": {
            "typescript_src/components/User.tsx": "charlie"
        },
