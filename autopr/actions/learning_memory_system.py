...
class LearningMemorySystem:
    def __init__(self, db_path: str = "autopr_memory.db"):
    ...
    def record_fix_pattern(self, inputs: MemoryInputs) -> bool:
        ...
        try:
            ...
            if existing:
                ...
                new_success_rate = ((success_rate * usage_count) + (1 if inputs.success else 0)) / new_usage_count
                ...
                """, (
                    inputs.comment_type, file_ext, inputs.fix_applied,
                    inputs.context.get('fix_code', ''), 1.0 if inputs.success else 0.0, 1, datetime.now()
                ))
    ...
    def record_user_preference(self, user_id: str, preference_type: str, preference_value: str, confidence: float = 1.0) -> bool:
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (?, ?, ?, ?, ?)
            """, (user_id, preference_type, preference_value, confidence, datetime.now()))
    ...
    def get_fix_recommendations(self, comment_type: str, file_path: str = None) -> List[Dict[str, Any]]:
    ...
    def _detect_coding_style(self, files: List[str]) -> Dict[str, Any]:
        ...
        return {
            "prefers_semicolons": style_indicators["semicolons"] > style_indicators["no_semicolons"],
            "quote_style": "single" if style_indicators["single_quotes"] > style_indicators["double_quotes"] else "double",
            "indentation": "spaces" if style_indicators["spaces"] > style_indicators["tabs"] else "tabs"
        }
    ...
    def _store_project_context(self, project_hash: str, patterns: Dict[str, Any]):
        ...
        try:
            for context_type, pattern_data in patterns.items():
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?)
                """, (project_hash, context_type, str(pattern_data), 1, 1.0, datetime.now()))
