...
Unified interface for multiple LLM providers (OpenAI, Anthropic, Mistral, Grok, Perplexity, etc.)
"""
...
class PerplexityProvider(BaseLLMProvider):
    ...
    def complete(self, request: LLMRequest) -> LLMResponse:
        try:
            response = self.client.chat.completions.create(
                model=request.model or self.default_model or "llama-3.1-sonar-large-128k-online",
                messages=[msg.dict() for msg in request.messages],
                temperature=request.temperature,
                max_tokens=request.max_tokens
            )
...
class TogetherAIProvider(BaseLLMProvider):
    ...
    def complete(self, request: LLMRequest) -> LLMResponse:
        try:
            response = self.client.chat.completions.create(
                model=request.model or self.default_model or "meta-llama/Llama-2-70b-chat-hf",
                messages=[msg.dict() for msg in request.messages],
                temperature=request.temperature,
                max_tokens=request.max_tokens
            )
...
class LLMProviderManager:
    ...
    def __init__(self, config: Dict[str, Any]):
        self.providers = {}
        self.fallback_order = config.get("fallback_order", ["openai", "anthropic", "mistral"])
        ...
        for provider_name, default_config in default_configs.items():
            user_config = provider_configs.get(provider_name, {})
    ...
    def complete(self, request: LLMRequest, provider: Optional[str] = None) -> LLMResponse:
        ...
        for provider_name in providers_to_try:
            ...
            try:
                response = provider_instance.complete(request)
    ...
    if _provider_manager is None:
        ...
        config = {
            ...
            "providers": {
                ...
                "perplexity": {
                    "default_model": os.getenv("AUTOPR_PERPLEXITY_MODEL", "llama-3.1-sonar-large-128k-online")
                },
                "together": {
                    "default_model": os.getenv("AUTOPR_TOGETHER_MODEL", "meta-llama/Llama-2-70b-chat-hf")
                }
            }
        }
...
def complete_chat(messages: List[Dict[str, str]], model: Optional[str] = None, provider: Optional[str] = None, **kwargs) -> LLMResponse:
    """Convenience function for chat completion."""
    llm_messages = [LLMMessage(role=msg["role"], content=msg["content"]) for msg in messages]
    request = LLMRequest(messages=llm_messages, model=model, **kwargs)
