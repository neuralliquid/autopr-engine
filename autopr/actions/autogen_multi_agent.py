...
class AutoGenAgentSystem:
    ...
    def analyze_and_fix_comment(self, inputs: AutoGenInputs) -> AutoGenOutputs:
        ...
        try:
            ...
            if "security" in inputs.comment_body.lower() or "vulnerability" in inputs.comment_body.lower():
            ...
            return AutoGenOutputs(
                success=False,
                error_message=f"AutoGen collaboration failed: {str(e)}"
            )
    ...
    def _extract_results_from_chat(self, chat_result, inputs: AutoGenInputs) -> AutoGenOutputs:
        ...
        try:
            ...
            analysis = self._analyze_conversations(agent_conversations)
            recommendations = self._extract_recommendations(agent_conversations)
            fix_code = self._extract_fix_code(agent_conversations)
            consensus = self._extract_consensus(agent_conversations)
            ...
            return AutoGenOutputs(
                success=False,
                error_message=f"Failed to extract results: {str(e)}"
            )
    ...
    def _analyze_conversations(self, conversations: List[Dict[str, str]]) -> Dict[str, Any]:
        ...
        all_content = " ".join(conv["message"] for conv in conversations)
        key_terms = ["fix", "security", "performance", "bug", "improvement", "refactor"]
    ...
    def _extract_recommendations(self, conversations: List[Dict[str, str]]) -> List[str]:
        ...
        for conv in conversations:
            ...
            if any(phrase in content for phrase in ["recommend", "suggest", "should", "consider"]):
                ...
                sentences = conv["message"].split('.')
                for sentence in sentences:
                    if any(phrase in sentence.lower() for phrase in ["recommend", "suggest", "should", "consider"]):
    ...
    def _extract_consensus(self, conversations: List[Dict[str, str]]) -> Optional[str]:
        ...
        for conv in reversed(conversations):  # Start from the end
            if conv["agent"] == "project_manager" and any(phrase in conv["message"].lower() for phrase in ["decision", "consensus", "final", "conclusion"]):
                return conv["message"]
...
def autogen_multi_agent_action(inputs: AutoGenInputs) -> AutoGenOutputs:
    ...
    try:
        ...
        return AutoGenOutputs(
            success=False,
            error_message=f"AutoGen system failed: {str(e)}"
        ) 