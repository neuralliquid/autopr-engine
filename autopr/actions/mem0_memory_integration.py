...
class Mem0MemoryManager:
    ...
    def get_insights(self, inputs: Mem0MemoryInputs) -> Mem0MemoryOutputs:
        ...
        try:
            ...
            fix_memories = [m for m in all_memories if m.get("metadata", {}).get("memory_type") == "fix_pattern"]
            if fix_memories:
            ...
            pref_memories = [m for m in all_memories if m.get("metadata", {}).get("memory_type") == "user_preference"]
            if pref_memories:
                insights.append(f"Learned {len(pref_memories)} user preferences")
            
            # Project-specific insights
            project_memories = [m for m in all_memories if m.get("metadata", {}).get("memory_type") == "project_context"]
            if project_memories:
            ...
            return Mem0MemoryOutputs(
                success=False,
                error_message=f"Failed to get insights: {str(e)}"
            )
    ...
    def record_fix_pattern(self, comment_type: str, fix_type: str, success: bool, user_id: str, context: Dict[str, Any]) -> bool:
    ...
    def record_user_preference(self, user_id: str, preference_type: str, preference_value: str, context: Dict[str, Any]) -> bool:
    ...
    def get_relevant_patterns(self, comment_type: str, file_path: str, user_id: str) -> List[Dict[str, Any]]:
...
def mem0_memory_action(inputs: Mem0MemoryInputs) -> Mem0MemoryOutputs:
    ...
    try:
        ...
        return Mem0MemoryOutputs(
            success=False,
            error_message=f"Mem0 operation failed: {str(e)}"
        ) 