...
class AxoloIntegration:
    ...
    async def create_pr_channel(self, pr_data: Dict[str, Any]) -> AxoloPRChannel:
        ...
        logger.info("PR channel created", 
    ...
    async def post_autopr_analysis(self, channel: AxoloPRChannel, analysis_result: Dict[str, Any]):
        ...
        if analysis_result.get('next_steps'):
            next_steps_block = self._create_next_steps_block(analysis_result['next_steps'])
            message['blocks'].append(next_steps_block)
        
        # Post to Slack
        await self._post_slack_message(message)
        
        logger.info("Analysis posted to channel", 
                   channel_id=channel.channel_id,
                   issues_count=len(analysis_result.get('issues_found', [])))
    
    async def link_linear_issues(self, channel: AxoloPRChannel, issues: List[Dict[str, Any]]):
        ...
        logger.info("Linear issues linked to channel",
                   channel_id=channel.channel_id,
                   issues_count=len(issues))
    
    async def setup_ai_assignments(self, channel: AxoloPRChannel, assignments: List[Dict[str, Any]]):
        ...
        logger.info("AI assignments posted to channel",
                   channel_id=channel.channel_id,
    ...
    async def _handle_analyze_command(self, command_data: Dict[str, Any]):
        ...
        if not pr_data:
            await self._post_error_response(channel_id, "Could not find PR to analyze. Please provide a PR URL or use this command in a PR channel.")
            return
        ...
        await self._post_slack_response(channel_id, f"🤖 Starting AutoPR analysis for PR #{pr_data['pr_number']}...")
    ...
    async def _handle_status_command(self, command_data: Dict[str, Any]):
        ...
        if not pr_data:
            await self._post_error_response(channel_id, "This command must be used in a PR channel.")
            return
            ...
            'blocks': [
                ...
                },
                {
                    ...
                    'fields': [
                        ...
                        {'type': 'mrkdwn', 'text': f"*Last Analysis:*\n{status.get('last_analysis', 'Never')}"},
                        {'type': 'mrkdwn', 'text': f"*Issues Count:*\n{status.get('issues_count', 0)}"}
                    ]
                }
            ]
        }
    ...
    async def _handle_assign_ai_command(self, command_data: Dict[str, Any]):
        ...
        if not text:
            await self._post_error_response(channel_id, "Please specify an AI tool: coderabbit, copilot, security, or autopr")
            return
        ...
        if ai_tool not in valid_tools:
            await self._post_error_response(channel_id, f"Unknown AI tool. Valid options: {', '.join(valid_tools)}")
            return
        ...
        await self._post_slack_response(channel_id, f"✅ {ai_tool.title()} assigned to PR #{pr_data['pr_number']}")
    ...
    async def _background_reminder_service(self):
        ...
        while True:
            try:
                ...
                # End of day summary
                elif current_time == self.config.reminder_schedule['end_of_day_summary']:
    ...
    async def _send_daily_standup_summary(self):
        ...
        logger.info("Daily standup summary sent", 
    ...
    def _format_analysis_summary(self, analysis_result: Dict[str, Any]) -> str:
        ...
        if analysis_result.get('platform_detected') and analysis_result.get('confidence_score', 0) > 0.8:
            summary_parts.append(f"**Recommendation**: High confidence detection - proceed with automated assignments")
    ...
    def _create_issues_block(self, issues: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for issue in issues[:5]:  # Limit to 5 issues
            issues_text.append(f"• **{issue.get('severity', 'Medium')}**: {issue.get('title', 'Unknown Issue')}")
            ...
            if self.slack_client:
                ...
                async with self.session.post(self.config.slack_webhook, json=message) as response:
                    if response.status != 200:
                        logger.error("Failed to post Slack message", status=response.status)
