...
class MetricsCollector:
    def __init__(self, db_path: str = "autopr_metrics.db"):
    ...
    def record_metric(self, metric_name: str, value: float, metadata: Dict[str, Any] = None, 
                     session_id: str = None, user_id: str = None, comment_id: str = None):
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (metric_name, value, json.dumps(metadata or {}), session_id, user_id, comment_id))
        
        conn.commit()
        conn.close()
    
    def record_event(self, event_type: str, event_data: Dict[str, Any], success: bool,
                    duration_ms: int = None, user_id: str = None, comment_id: str = None, pr_number: int = None):
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """, (event_type, json.dumps(event_data), success, duration_ms, user_id, comment_id, pr_number))
        
        conn.commit()
        conn.close()
    
    def record_user_feedback(self, user_id: str, rating: int, feedback_text: str = None,
                           category: str = None, autopr_action: str = None, comment_id: str = None):
        """Record user feedback."""
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        
        cursor.execute("""
            INSERT INTO user_feedback (user_id, comment_id, rating, feedback_text, category, autopr_action)
            VALUES (?, ?, ?, ?, ?, ?)
        """, (user_id, comment_id, rating, feedback_text, category, autopr_action))
        
        conn.commit()
        conn.close()
    
    def record_benchmark(self, benchmark_name: str, test_case: str, expected_result: str,
                        actual_result: str, success: bool, duration_ms: int = None,
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (benchmark_name, test_case, expected_result, actual_result, success, duration_ms, provider, model))
    ...
    def get_benchmark_results(self, benchmark_name: str = None, timeframe: str = "30d") -> Dict[str, Any]:
    ...
    def generate_report(self, timeframe: str = "7d") -> Dict[str, Any]:
        ...
        trends = {}
        key_metrics = ["fix_success_rate", "user_satisfaction_score", "avg_response_time", "error_rate"]
    ...
    def _generate_recommendations(self, metrics: EvaluationMetrics, trends: Dict[str, Any]) -> List[str]:
        ...
        if metrics.avg_response_time > 5.0:
            recommendations.append("Response time is high - consider using faster LLM providers or caching")
